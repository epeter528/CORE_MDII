...
//    fprintf(fplog,"%s%f\t%d\n","msd_mean",msd_mean[i],rank);
 ...
 //   };
    ...
    for(i=1;i<=re->nrepl;i++){
            ...
            probability[i] = exp(-2.0*msd_mean[i-1]);
...
//    } // originally this was the loop over the replicas
...
//    print_prob(fplog,"pr",re->nrepl,prob);
    ...
    for(i=1;i<=re->nrepl;i++){
      ...
      printf("%s\t%d\t%f\n","replica probability",i,probability[i]);
...
//    {
    ...
    if(MULTIMASTER(cr)){
        ...
        MPI_Bcast(&exchange,sizeof(int),MPI_INT,MASTERRANK(cr),
                  cr->mpi_comm_mygroup);
